...
=============================================================
     4     23.2 MiB     23.2 MiB           1   @profile
     5                                         def ant_path(width, height):
     6     24.2 MiB      1.0 MiB           1       image = Image.new("1", (width, height), color=1)  # Создаем изображение с белыми клетками (значение 1)
     7     24.3 MiB      0.0 MiB           1       pixels = image.load()
     8     24.3 MiB      0.0 MiB           1       x, y = width // 2, height // 2  # Начальные координаты муравья
     9     24.3 MiB      0.0 MiB           1       direction = 0  # Начальное направление (0 - вверх, 1 - вправо, 2 - вниз, 3 - влево)
    10
    11     24.3 MiB      0.0 MiB       35694       while 0 <= x < width and 0 <= y < height:
    12     24.3 MiB      0.0 MiB       35693           if pixels[x, y] == 1:  # Если клетка белая
    13     24.3 MiB      0.0 MiB       19688               direction = (direction + 1) % 4  # Поворачиваем на 90° по часовой стрелке
    14     24.3 MiB      0.0 MiB       19688               pixels[x, y] = 0  # Инвертируем пиксель (черный)
    15                                                 else:  # Если клетка черная
    16     24.3 MiB      0.0 MiB       16005               direction = (direction - 1) % 4  # Поворачиваем на 90° против часовой стрелки
    17     24.3 MiB      0.0 MiB       16005               pixels[x, y] = 1  # Инвертируем пиксель (белый)
    18
    19                                                 # Перемещаем муравья вперед на одну клетку в зависимости от направления
    20     24.3 MiB      0.0 MiB       35693           if direction == 0:  # Вверх
    21     24.3 MiB      0.0 MiB        8680               y -= 1
    22     24.3 MiB      0.0 MiB       27013           elif direction == 1:  # Вправо
    23     24.3 MiB      0.0 MiB        8667               x += 1
    24     24.3 MiB      0.0 MiB       18346           elif direction == 2:  # Вниз
    25     24.3 MiB      0.0 MiB        9166               y += 1
    26     24.3 MiB      0.0 MiB        9180           elif direction == 3:  # Влево
    27     24.3 MiB      0.0 MiB        9180               x -= 1
    28
    29                                             # Сохраняем изображение
    30     25.2 MiB      0.9 MiB           1       image.save("ant_path2.png")
    31     25.2 MiB      0.0 MiB           1       return image
```
